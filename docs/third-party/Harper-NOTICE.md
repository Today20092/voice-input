...
Development prerequisites are Rust 1.94.0 and the `aarch64-linux-android` target in addition to the existing Android NDK. CMake invokes Cargo using the NDK compiler; native intermediates remain in the Android build directory. No JavaScript or WebAssembly host is added.
